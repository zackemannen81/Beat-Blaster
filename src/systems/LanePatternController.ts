...
type LaneSpawn = SpawnBase & {
  kind: 'lane'
  lanes: LaneRef | LaneRef[]
  count: number
 }

type LaneHopperSpawn = SpawnBase & {
  kind: 'lane_hopper'
  from: LaneRef
  to: LaneRef
  hopEvery?: number
...
type TeleporterSpawn = SpawnBase & {
  kind: 'teleporter'
  count: number
  lanes?: LaneRef | LaneRef[]
 }

type LaneFloodSpawn = SpawnBase & {
  kind: 'lane_flood'
  lane: LaneRef
  widthRatio?: number
  height?: number
...
function buildPattern(difficulty: DifficultyProfileId): BeatStep[] {
