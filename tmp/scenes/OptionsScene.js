...
import { loadOptions, saveOptions, detectGameplayModeOverride, resolveGameplayMode } from '../systems/Options';
export default class OptionsScene extends Phaser.Scene {
    ...
    create(data = {}) {
        ...
        const stepY = 0.055;
        const rowCount = 14;
        this.entries = Array.from({ length: rowCount }, (_, i) => this.add.text(width / 2, height * (baseY + stepY * i), '', { fontFamily: 'UiFont', fontSize: '18px' }).setOrigin(0.5));
        this.render();
    ...
    render() {
        const trackId = this.registry.get('selectedTrackId') || 'default';
        ...
        const crosshairLabel = this.opts.crosshairMode === 'pointer'
            ? 'Pointer'
    ...
    adjust(dir) {
        ...
        switch (this.cursor) {
            case 0:
                this.opts.musicVolume = Phaser.Math.Clamp(this.opts.musicVolume + step * 0.05, 0, 1);
                break;
            case 1:
                this.opts.sfxVolume = Phaser.Math.Clamp(this.opts.sfxVolume + step * 0.05, 0, 1);
                break;
            case 2:
                this.opts.metronome = !this.opts.metronome;
                break;
            case 3:
                this.opts.highContrast = !this.opts.highContrast;
                break;
            case 4: {
            ...
            case 6: {
                const order = ['click', 'hold_quantized', 'hold_raw'];
