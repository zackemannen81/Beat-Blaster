...
export default class ResultScene extends Phaser.Scene {
    ...
    create(data = {}) {
        ...
        this.add.text(width / 2, height * 0.55, `Score: ${data.score ?? 0}\nAccuracy: ${accuracyText}`, { fontFamily: 'UiFont, sans-serif', fontSize: '18px', color: '#a0e9ff', align: 'center' }).setOrigin(0.5);
        // Save to local leaderboard (prompt name once)
        const score = data.score ?? 0;
        const trackId = this.registry.get('selectedTrackId') || 'unknown';
