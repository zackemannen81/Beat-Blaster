...
export default class AudioAnalyzer extends Phaser.Events.EventEmitter {
    constructor(scene) {
        ...
        this.smoothing = 0.5; // Increased for more stable detection
        this.fftSize = 2048;
        this.started = false;
        // Rolling windows
        this.historyLow = []; // Bass drum
        this.historyMid = []; // Snare body
        this.historyHigh = []; // Snare snap
        this.HIST = 30; // Reduced history for faster response
        this.MIN_INTERVAL_MS = 100;
     ...
     */
    fallbackAnalyzerSetup(webAudioSound) {
    ...
    detectBeats(bands) {
        ...
        const isBeat = (band, threshold, minInterval = 80) => {
        ...
        if (this.historyLow.length > this.HIST)
            this.historyLow.shift();
        if (this.historyMid.length > this.HIST)
            this.historyMid.shift();
        if (this.historyHigh.length > this.HIST)
