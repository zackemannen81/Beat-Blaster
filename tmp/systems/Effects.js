...
export default class Effects {
    ...
    plasmaCharge(x, y, rotation) {
        if (this.reducedMotion)
            return;
        if (!this.scene.textures.exists('bullet_plasma_charge_0'))
            return;
    ...
    clearPlasmaTrail(bullet) {
        const emitters = bullet.getData('plasmaTrailEmitters');
        emitters?.forEach((emitter) => emitter.destroy());
        bullet.setData('plasmaTrailEmitters', undefined);
        const timer = bullet.getData('plasmaTrailTimer');
        timer?.remove(false);
