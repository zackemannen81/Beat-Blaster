...
const VALID_FIRE_MODES = ['click', 'hold_raw', 'hold_quantized'];
const VALID_CROSSHAIR_MODES = ['pointer', 'fixed', 'pad-relative'];
function normalizeGameplayMode(value) {
    if (typeof value === 'string') {
        const normalized = value.trim().toLowerCase();
        if (['vertical', 'v', '1', 'true', 'yes'].includes(normalized))
            return 'vertical';
        if (['omni', 'o', '0', 'false', 'no', 'horizontal'].includes(normalized))
            return 'omni';
    }
...
function withDefaults(raw) {
    ...
    if (!VALID_FIRE_MODES.includes(merged.fireMode))
        merged.fireMode = DEFAULT_OPTIONS.fireMode;
    const mode = normalizeGameplayMode(raw?.gameplayMode);
    ...
    merged.crosshairMode = VALID_CROSSHAIR_MODES.includes(crosshair)
        ? crosshair
...
export function detectGameplayModeOverride() {
    ...
    if (typeof window !== 'undefined') {
        const globalValue = window.VERTICAL_MODE ?? window.verticalMode;
...
export function resolveGameplayMode(baseMode) {
