...
export default class PlayerSkin {
    constructor(scene, host) {
        ...
        this.expectMaxVX = 260;
        this.expectMaxVY = 340;
        // Använder Vector2 för enklare matte, men din `localToWorld` fungerar också
        this.tailOffset = new Phaser.Math.Vector2(0, this.size * 1.08);
        this.wingLOffset = new Phaser.Math.Vector2(-this.size * 0.95, this.size * 0.35);
        this.wingROffset = new Phaser.Math.Vector2(this.size * 0.95, this.size * 0.35);
        ...
        this.follow = () => {
            ...
            const targetTilt = Phaser.Math.Clamp((vx / this.expectMaxVX) * maxTiltRad, -maxTiltRad, maxTiltRad);
            ...
            if (this.emMain)
                this.emMain.setPosition(tailPos.x, tailPos.y + 5);
            if (this.emWingL)
                this.emWingL.setPosition(wingLPos.x, wingLPos.y + 15);
            if (this.emWingR)
                this.emWingR.setPosition(wingRPos.x, wingRPos.y + 15);
            if (this.particleManager)
                this.particleManager.setPosition(tailPos.x, tailPos.y);
    ...
    createThrusters() {
        ...
        */
        ...
        particleManager.startFollow(this.host.body);
