...
import { enemyStyles } from '../config/enemyStyles';
...
export default class Spawner {
    ...
    setDifficulty(config) {
        if (typeof config.hpMultiplier === 'number' && config.hpMultiplier > 0)
            this.hpMultiplier = config.hpMultiplier;
        if (typeof config.bossHpMultiplier === 'number' && config.bossHpMultiplier > 0)
            this.bossHpMultiplier = config.bossHpMultiplier;
        if (!this.laneManager && typeof config.laneCount === 'number' && config.laneCount >= 3) {
            this.laneCount = Math.floor(config.laneCount);
        }
    }
    setLaneManager(manager) {
    ...
    spawnVerticalLane(type, laneIndex, count = 1, speedMultiplier = 1, hpMultiplier, waveId) {
        const spawned = [];
    ...
    spawnLaneHopper(type, laneA, laneB, speedMultiplier = 1, hopEveryBeats = 1, hpMultiplier, waveId) {
        if (this.laneCount <= 0)
            return [];
    ...
    spawnSineWave(type, count, amplitudePx, wavelengthPx, speedMultiplier = 1, hpMultiplier, waveId) {
        const spawned = [];
    ...
    spawnVFormation(type, size, spreadPx, speedMultiplier = 1, hpMultiplier, waveId) {
        const spawned = [];
    ...
    spawnWave(descriptor, anchor, options = {}) {
        const formation = descriptor.formation;
    ...
    spawnLaneWave(descriptor, anchor, waveId, skipTelegraph) {
        ...
        const spawned = this.spawnVerticalLane(descriptor.enemyType, laneIndex, count, speedMul, descriptor.hpMultiplier, waveId);
    ...
    spawnTeleporterWave(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const laneIndex = typeof params.laneIndex === 'number' ? params.laneIndex : Math.floor(this.laneCount / 2);
    ...
    spawnLaneFloodWave(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
    ...
    spawnLaneHopperDescriptor(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        ...
        return this.spawnLaneHopper(descriptor.enemyType, laneA, laneB, speedMul, hopEvery, descriptor.hpMultiplier, waveId);
    }
    spawnSineWaveDescriptor(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const count = descriptor.count ?? 4;
        const amp = params.amplitude ?? 140;
        const wavelength = params.wavelength ?? 360;
        const speedMul = descriptor.speedMultiplier ?? 1;
        const enemies = this.spawnSineWave(descriptor.enemyType, count, amp, wavelength, speedMul, descriptor.hpMultiplier, waveId);
        if (!skipTelegraph && descriptor.telegraph) {
            const first = enemies[0];
            if (first) {
    ...
    spawnWeaverWave(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const count = descriptor.count ?? 4;
    ...
    spawnVWaveDescriptor(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const size = descriptor.count ?? 5;
        const spread = params.spread ?? 65;
        const speedMul = descriptor.speedMultiplier ?? 1;
        const enemies = this.spawnVFormation(descriptor.enemyType, size, spread, speedMul, descriptor.hpMultiplier, waveId);
        if (!skipTelegraph && descriptor.telegraph) {
    ...
    spawnFormationDancerWave(descriptor, anchor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
    ...
    spawnMirrorerWave(descriptor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const laneIndex = typeof params.laneIndex === 'number' ? params.laneIndex : Math.floor(this.laneCount / 2);
        const centerX = this.getLaneCenterX(laneIndex);
    ...
    spawnSwirlWave(descriptor, anchor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
    ...
    spawnCircleWave(descriptor, anchor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
    ...
    spawnSpiralWave(descriptor, anchor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const count = descriptor.count ?? 6;
    ...
    spawnSpiralDropWave(descriptor, anchor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
        const count = descriptor.count ?? 8;
    ...
    spawnBurstWave(descriptor, anchor, waveId, skipTelegraph) {
        const params = descriptor.formationParams ?? {};
    ...
    createEnemy(config) {
        ...
        const sprite = this.group.get(config.x, config.y, 'gameplay', placeholderFrame);
        sprite.setActive(true).setVisible(true);
        sprite.body.enable = true;
        sprite.setData('etype', config.type);
        const healthBar = this.scene.add.graphics();
        sprite.setData('healthBar', healthBar);
        const balance = this.scene.registry.get('balance');
